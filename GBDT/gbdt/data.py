...
class DataSet:
    ...
    回归问题也统一使用label
    field_names: tuple,(特征名)存放特征名
    instances: dict,{index:instance};  instance: dict,{特征名:特征值}
    distinct_valueset: dict,{特征名：set(特征对应的所有数值)}  存放数值特征名及其对应的所有数值
    field_type：dict,{特征名：set(特征所对应的所有类型)}
    """
    def __init__(self, filename):  # just for csv data format
        ...
        for line in open(filename):
            ...
            if line_cnt == 0:  # csv head
                self.field_names = tuple(fields)  # 得到特征向量名
            else:
    ...
    def _construct_instance(self, fields):
        ...
        for i in range(0, len(fields)):
            ...
            real_type_mark = self.is_real_type_field(field_name)  # 判断特征类型是否是数值类型
            if real_type_mark:     # 数值类型
                try:
                    instance[field_name] = float(fields[i])
                    self.distinct_valueset[field_name].add(float(fields[i]))  # 将每一类的所有数值保存下来
                except ValueError:
...
if __name__ == "__main__":
    ...
    # print data.distinct_valueset.__len__()
